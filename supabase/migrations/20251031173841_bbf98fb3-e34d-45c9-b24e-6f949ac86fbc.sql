...
-- Add length constraints to profiles table for input validation
...
ALTER TABLE public.donations
  ...
  ADD CONSTRAINT donation_amount_reasonable CHECK (amount <= 1000000),
  ADD CONSTRAINT donation_message_length CHECK (char_length(message) <= 1000);

-- NGOs table validation
ALTER TABLE public.ngos
  ADD CONSTRAINT ngo_name_length CHECK (char_length(name) BETWEEN 1 AND 200),
  ADD CONSTRAINT ngo_description_length CHECK (char_length(description) <= 2000),
  ADD CONSTRAINT ngo_location_length CHECK (char_length(location) <= 200),
  ADD CONSTRAINT ngo_email_format CHECK (contact_email IS NULL OR contact_email ~* '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$'),
  ADD CONSTRAINT ngo_website_format CHECK (website IS NULL OR website ~* '^https?://');

-- Categories table validation
ALTER TABLE public.categories
  ADD CONSTRAINT category_name_length CHECK (char_length(name) BETWEEN 1 AND 100),
...
ALTER TABLE public.impact_metrics
  ADD CONSTRAINT metric_name_length CHECK (char_length(metric_name) BETWEEN 1 AND 100),
  ADD CONSTRAINT metric_value_positive CHECK (metric_value >= 0);
